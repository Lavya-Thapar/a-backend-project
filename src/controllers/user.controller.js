...
const  getUserChannelProfile = asyncHandler(async (req,res)=>{
    ...
    if(!(username?.trim()))
    {
        throw new ApiError(400,"No username found from params")
    }

    const channelFound = await User.aggregate([
        {
            $match:{
                username: username?.toLowerCase()
            } 
        },
        // till here , we found the user with this username
        {
            $lookup: {
                from : "subscriptions",
                localField: "_id",
        ...
        {
            // $addFields is an operator that adds new fields to existing data object
            $addFields: {
                subscribersCount: {
                    $size: "$subscribers"
                },
            ...
            //now , these fields are added as fields to the object
        },
        {
            $project: {
                fullname: 1,
    ...
    console.log("channel details: ", channelFound)
    ...
    const user = User.aggregate([
        {
            $match: {
                ...
                //NOTE: req.user._id will only give a string , not the actual mongoDB id,
                // hence we need to build the actual mongoDB id
            },
        ...
        { 
            $lookup: {
                ...
                pipeline: [
                    ...
                    {
                        $addFields: {
                            //instead of adding another owner field, over-write the owner field
                            owner: {
                                $first: "$owner"
                            }
                        }
